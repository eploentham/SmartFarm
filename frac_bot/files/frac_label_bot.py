...
GEMINI_PROMPT = """You are reading a Thai agricultural chemical product label.
The text may be mixed Thai and English. Return ONLY valid JSON, no markdown,
no extra text:
{
  "product_name": "commercial name, e.g. Score 250EC or ไอวี่กรีน",
  "active_ingredient": "chemical name in English, e.g. Difenoconazole",
  "appears_to_be": "Fungicide | Insecticide | Other",
  "concentration": "e.g. 250 g/L EC, or empty string if unknown",
  "confidence": 0.0
}
If you cannot read the active ingredient, set it to "" and lower confidence.
"""
...
def analyze_label(image_path: Path) -> dict:
    ...
    resp = model.generate_content([GEMINI_PROMPT, img])
    ...
    try:
        ...
        return {"product_name": "", "active_ingredient": "",
                "appears_to_be": "Other", "concentration": "", "confidence": 0.0}
...
async def handle_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📥 รับรูปแล้ว กำลังอ่านฉลาก...")
    ...
    kind_guess = parsed.get("appears_to_be", "Other")
    ...
    lines.append(f"📦 สินค้า: {product or '(อ่านไม่ออก)'}")
    lines.append(f"🧪 สารออกฤทธิ์: {ai or '(อ่านไม่ออก)'}")
    lines.append(f"🤖 Gemini เดาว่าเป็น: {kind_guess} (มั่นใจ {conf:.0%})")
