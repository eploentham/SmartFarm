...
# -*- coding: utf-8 -*-
"""
detect_worker_zone3_cam2_v2.py  (SNAPSHOT version — ffmpeg grabs one frame / 15s)
Worker (คนสวน) presence detector for ONE orchard CCTV camera.

Camera: Hamrol/XMEye @ 192.168.0.241, plot DURIAN-A3 (cam2).

Flow every SAMPLE_INTERVAL_SEC:
  ffmpeg grab 1 frame -> YOLO detect person -> if found: log + snapshot.

One script per camera. Copy -> detect_worker_zone3_cam3_v2.py, edit the CONFIG block.
...
# CONFIG
# ======================================================================
CAMERA_CODE = "CAM-A3-02"          # <= 20 chars, MUST be unique per camera
...
RTSP_HOST    = _env("RTSP_HOST_A3_02", "RTSP_HOST", default="192.168.0.241")
