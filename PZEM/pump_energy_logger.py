#!/usr/bin/env python3
"""
pump_energy_logger.py — MQTT -> MariaDB logger for the PZEM pump nodes.

Flow:
    subscribe smartfarm/pump/#  ->  validate pump_id against m_pump
    ->  buffer  ->  batch insert into t_pump_energy
...
Details:
    ...
    - pump_id is checked against the m_pump whitelist to avoid FK error 1452;
      the whitelist is refreshed periodically so new pumps get picked up.
...
# Make sibling folders importable regardless of layout: works whether config.py
# and the helpers sit next to this file (flat) or in ../config and ../helpers
# (as on the pn64 server: scripts/, config/, helpers/).
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/pump_energy_logger.log"),
        logging.StreamHandler(),
    ],
)
log = logging.getLogger("pump_logger")
...
def on_reading(topic, payload):
    ...
    row = db_helper.row_from_payload(payload, _reading_at(payload, received_at))
    with _lock:
        _buffer.append(row)
    maybe_flush()
